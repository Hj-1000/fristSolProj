...
public class ReplyServiceImpl implements ReplyService{
    ...
    public Reply replyRegister(ReplyDTO replyDTO, String userID) {
        ...
        Member member =
        memberRepository.findById(replyDTO.getMemberDTO().getMno()).orElseThrow(EntityNotFoundException::new);

        log.info("서비스에 들어온 댓글 작성자" + member);

        Item item =
                itemRepository.findById(replyDTO.getItemDTO().getIno()).orElseThrow(EntityNotFoundException::new);
